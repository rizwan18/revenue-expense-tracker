...
export function DataBackupCard({ onImported }: { onImported?: () => void }) {
  ...
  async function handleExport() {
    ...
    setExporting(true);
    try {
      const blob = await api.get<Blob>("/data/export");
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `revenue-expense-tracker-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      ...
      const result = await api.post<ImportResult>("/data/import", { csv, dryRun: true, includeProfile });
  ...
  async function confirmImport(csv: string) {
    ...
    setImportError(null);
    try {
      const result = await api.post<ImportResult>("/data/import", { csv, dryRun: false, includeProfile });
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="mt-3">
            ...
            <Button variant="secondary" onClick={() => fileInput.current?.click()} disabled={stage.name === "checking"}>
